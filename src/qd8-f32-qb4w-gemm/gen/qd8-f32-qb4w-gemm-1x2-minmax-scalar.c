    ...
    const struct xnn_qd8_quantization_params quantization_params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  do {
    const int32_t vksum0 = unaligned_indexed_load_f32(w, 0);
    const int32_t vksum1 = unaligned_indexed_load_f32(w, 1);
    const int32_t vinput_zero_point0 = quantization_params[0].zero_point;
    float vout0x0 = vksum0 * (float) vinput_zero_point0;
    float vout0x1 = vksum1 * (float) vinput_zero_point0;
    w = (const int32_t*) w + 2;
