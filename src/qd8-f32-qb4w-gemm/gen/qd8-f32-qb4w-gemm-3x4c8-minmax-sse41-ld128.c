    ...
    const struct xnn_qd8_quantization_params quantization_params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  do {
    ...
    #ifdef SCALE_SIMD_FP16
      const __m128 vfilter_output_scale0123 = fp16_ieee_to_fp32_value_sse(w);
      w = (const uint16_t*) w + 4;
    #else
      const __m128 vfilter_output_scale0123 = _mm_set_ps(
        fp16_ieee_to_fp32_value(((const uint16_t*) w)[3]),
        fp16_ieee_to_fp32_value(((const uint16_t*) w)[2]),
        fp16_ieee_to_fp32_value(((const uint16_t*) w)[1]),
        fp16_ieee_to_fp32_value(((const uint16_t*) w)[0])
      );
      w = (const uint16_t*) w + 4;
    #endif
